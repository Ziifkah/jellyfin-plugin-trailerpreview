...
public class TrailerPreviewController : ControllerBase
{
    ...
    private string BuildClientScript(PluginConfiguration config)
    {
        ...
        sb.AppendLine("        videoElement.src = `https://www.youtube-nocookie.com/embed/${youtubeId}?autoplay=1&mute=1&controls=0&loop=1&playlist=${youtubeId}&playsinline=1&rel=0&modestbranding=1&enablejsapi=1&origin=${window.location.origin}`;");
        ...
        sb.AppendLine("          videoElement.addEventListener('load', () => {");
        sb.AppendLine("            setTimeout(() => {");
        sb.AppendLine("              try {");
        sb.AppendLine("                if (videoElement && videoElement.contentWindow) {");
        sb.AppendLine("                  videoElement.contentWindow.postMessage(JSON.stringify({ event: 'command', func: 'unMute' }), '*');");
        sb.AppendLine("                  videoElement.contentWindow.postMessage(JSON.stringify({ event: 'command', func: 'setVolume', args: [CONFIG.audioVolume / 100] }), '*');");
        sb.AppendLine("                }");
        sb.AppendLine("              } catch (error) {");
        sb.AppendLine("                debugLog('Error controlling YouTube audio: ' + error.message);");
        sb.AppendLine("              }");
        sb.AppendLine("            }, 1000);");
        sb.AppendLine("          });");
        sb.AppendLine("        }");
