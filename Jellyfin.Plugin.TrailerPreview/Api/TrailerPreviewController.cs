...
public class TrailerPreviewController : ControllerBase
{
    ...
    private string BuildClientScript(PluginConfiguration config)
    {
        ...
        sb.AppendLine("        videoElement.id = `youtube-player-${Date.now()}`;");
        ...
        sb.AppendLine("        videoElement.src = `https://www.youtube-nocookie.com/embed/${youtubeId}?autoplay=1&mute=${muteParam}&controls=0&loop=1&playlist=${youtubeId}&playsinline=1&rel=0&modestbranding=1&showinfo=0&iv_load_policy=3&hd=1&enablejsapi=1&origin=${window.location.origin}`;");
        ...
        sb.AppendLine();
        sb.AppendLine("        // Force HD quality via YouTube API");
        sb.AppendLine("        const onYouTubeReady = (event) => {");
        sb.AppendLine("          try {");
        sb.AppendLine("            if (event.source === videoElement.contentWindow) {");
        sb.AppendLine("              const data = JSON.parse(event.data);");
        sb.AppendLine("              if (data.event === 'onReady') {");
        sb.AppendLine("                debugLog('YouTube player ready, setting quality to hd1080');");
        sb.AppendLine("                videoElement.contentWindow.postMessage(JSON.stringify({");
        sb.AppendLine("                  event: 'command',");
        sb.AppendLine("                  func: 'setPlaybackQuality',");
        sb.AppendLine("                  args: ['hd1080']");
        sb.AppendLine("                }), '*');");
        sb.AppendLine("                // Try multiple quality levels");
        sb.AppendLine("                setTimeout(() => {");
        sb.AppendLine("                  videoElement.contentWindow.postMessage(JSON.stringify({");
        sb.AppendLine("                    event: 'command',");
        sb.AppendLine("                    func: 'setPlaybackQualityRange',");
        sb.AppendLine("                    args: ['hd1080', 'hd720']");
        sb.AppendLine("                  }), '*');");
        sb.AppendLine("                }, 500);");
        sb.AppendLine("              }");
        sb.AppendLine("            }");
        sb.AppendLine("          } catch (e) {");
        sb.AppendLine("            // Ignore parse errors");
        sb.AppendLine("          }");
        sb.AppendLine("        };");
        sb.AppendLine("        window.addEventListener('message', onYouTubeReady);");
        sb.AppendLine("        debugLog(`YouTube iframe created with mute=${muteParam}, audio enabled: ${CONFIG.enableAudio}, quality: HD forced`);");
        sb.AppendLine("      }");
