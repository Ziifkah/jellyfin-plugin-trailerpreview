...
public class TrailerPreviewController : ControllerBase
{
    ...
    private string BuildClientScript(PluginConfiguration config)
    {
        ...
        sb.AppendLine("        const autoplay = CONFIG.enableAudio ? '1' : '0';");
        sb.AppendLine("        const mute = CONFIG.enableAudio ? '0' : '1';");
        sb.AppendLine("        videoElement.src = `https://www.youtube.com/embed/${youtubeId}?autoplay=1&mute=${mute}&controls=0&modestbranding=1&rel=0&showinfo=0`;");
        sb.AppendLine("        videoElement.allow = 'autoplay; encrypted-media';");
