...
public class TrailerPreviewController : ControllerBase
{
    ...
    private string BuildClientScript(PluginConfiguration config)
    {
        ...
        sb.AppendLine("        videoElement.src = `https://www.youtube-nocookie.com/embed/${youtubeId}?autoplay=1&mute=1&controls=0&loop=1&playlist=${youtubeId}&playsinline=1&rel=0&modestbranding=1&enablejsapi=1`;");
        ...
        sb.AppendLine("              videoElement.contentWindow.postMessage(JSON.stringify({ event: 'command', func: 'setVolume', args: [CONFIG.audioVolume] }), '*');");
        sb.AppendLine("            }, 1000);");
        sb.AppendLine("          });");
        sb.AppendLine("        }");
        sb.AppendLine("      }");
